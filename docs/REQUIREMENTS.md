...
### フェーズ 1: 開発環境構築 & スキーマ設計 (pnpm / Prisma / OpenAPI)
*   **Environment Setup**: pnpm ワークスペースの構築。Node.js (API/Web), Prisma Studio のセットアップ。
*   **DB/API Schema**: 
    - `packages/database` で Prisma による SQLite テーブル定義。
    - `packages/openapi` で Zod による API 仕様定義。
*   **Persistence**: プロジェクト内の `storage/` ディレクトリと SQLite ファイル (`.db`) の管理設定。

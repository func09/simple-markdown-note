...
export const AppLayout: React.FC<AppLayoutProps> = React.memo(({ nav, list, main }) => {
  ...
  const handleLayoutChange = React.useCallback((layout: ResizablePrimitive.Layout) => {
    if (layoutMode === 'all') {
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-[#0f172a] text-slate-200">
      ...
      >
        ...
        {layoutMode === 'all' && (
          <ResizableHandle withHandle className="bg-slate-800/50 hover:bg-blue-500/50 transition-colors w-[2px]" />
        )}
        ...
        <ResizablePanel
          id={IDS.EDITOR}
          defaultSize={layoutMode === 'all' ? sanitizedAll[2] : (layoutMode === 'split' ? sanitizedSplit[1] : 100)}
          minSize="300px"
        >
          <main className="flex-1 h-full flex flex-col bg-[#0f172a] min-w-0" id="note-editor">
            {main}
          </main>
        </ResizablePanel>
      </ResizablePanelGroup>

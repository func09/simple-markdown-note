...
const MOCK_NOTES: Note[] = [
  {
    ...
    userId: 'user1',
    tags: [{ id: 't1', userId: 'user1', name: 'Japan', createdAt: '', updatedAt: '' }],
    deletedAt: null,
    isPermanent: false,
  ...
  },
  {
    ...
    userId: 'user1',
    tags: [{ id: 't2', userId: 'user1', name: 'Travel', createdAt: '', updatedAt: '' }],
    deletedAt: null,
    isPermanent: false,
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
...
describe('useOramaSearch', () => {
  ...
  it('should filter notes by query', () => {
    const { result } = renderHook(() => useOramaSearch(MOCK_NOTES, null, '日本'));
  ...
  it('should filter notes by tag', () => {
    const { result } = renderHook(() => useOramaSearch(MOCK_NOTES, 'Japan', ''));
  ...
  it('should filter untagged notes', () => {
    const { result } = renderHook(() => useOramaSearch(MOCK_NOTES, '__untagged__', ''));

...
import { useNotes, useOramaSearch } from "@/features/notes/hooks";
...
export const useDashboardState = () => {
  ...
  const isUntaggedSelected = filter === "untagged";

  const selectedTag = useMemo(() => {
    if (isUntaggedSelected) return "__untagged__";
    if (tagName) return tagName;
    return null;
  }, [isUntaggedSelected, tagName]);

  // URLから選択中のノートIDを導出
  const selectedNoteId = noteId || null;

  // サーバーからフィルタリング済みのノートリストを取得
  const { data: notes = [], isLoading: notesLoading } = useNotes({
    tag: tagName,
    scope: isTrashSelected ? "trash" : isUntaggedSelected ? "untagged" : "all",
  });
  ...
  const updateSelection = React.useCallback(
    (tag: string | null, isTrash: boolean, query: string = searchQuery) => {
      // 選択時点でのフィルタリング結果を同期的に取得し、先頭ノートを決定する
